...
4. **Per-slot production loop (order is mandatory)**
   ...
   - **4b — Image (required unless intake marks row text-only):** Run **`auto-image-generation`** immediately after 4a for that slot—**load that skill fully**: **`STYLE LOCK`** must lead **`prompt-master.txt`**, benchmark negatives included, **post-render visual QA** (no sci‑fi/holographic/anime drift for IT topics). Write **`posts/<post-id>/post-image.png`** (+ **`image-alt.txt`**) or **`article-hero.png`** per **`auto-image-generation`** → *Chaining*. If Gemini refuses, document in bundle under **`## Image`** as `STATUS: missing — reason` and escalate in **`APPROVAL.md`**.
